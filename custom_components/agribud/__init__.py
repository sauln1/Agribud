...
from .api import (
    PerenualApiClient,
    PerenualAuthError,
    PerenualConnectionError,
    PerenualApiError,
    PerenualRateLimitError,
)
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    api = PerenualApiClient(
        ...
        session,
        on_request=lambda endpoint: tracker.record(),  # noqa: ARG005
    )
    _LOGGER.info(
...
def _fire_data_changed(hass: HomeAssistant, kind: str, **extra: dict) -> None:
...
def _register_services(hass: HomeAssistant) -> None:  # noqa: C901, PLR0915

    async def handle_add_plant(call: ServiceCall) -> None:  # noqa: PLR0912
